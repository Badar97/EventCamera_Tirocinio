        ...
        for frame in recording["frames"]:
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    redDict = ft.getTotValues(face_landmarks)
                    
                    mp_drawing.draw_landmarks(image=annotated_image,landmark_list=landmark_pb2.NormalizedLandmarkList(landmark = landmark_list[0]),landmark_drawing_spec=mp_drawing.DrawingSpec(color=ld.BLUE_COLOR, thickness=1, circle_radius=1))
                    ...
                    cv2.rectangle(annotated_image,startLE,stopLE,ld.BLUE_COLOR,1)
                    ...
                    if(bool(redDict)):
                        ...
                        totDict["leftEye"] = {'xMin':startLE[0],'yMin':startLE[1],'xMax':stopLE[0],'yMax':stopLE[1]}
                        totDict["rightEye"] = {'xMin':startRE[0],'yMin':startRE[1],'xMax':stopRE[0],'yMax':stopRE[1]}
...
ft.writeJson(list,"dvSaveTotFrames")
ft.writeJson(bboxes,"totBoundingBoxes")
...
with AedatFile(ft.genPath+"dvSave.aedat4") as f:
    ...
    for event in events:
        ...
        string,bool2 = ft.filterTotEventCoords(event["x"],event["y"],startLE,stopLE,startRE,stopRE,startLI,stopLI)
        if bool2:
            dict["timestamp"], dict["x"], dict["y"], dict["polarity"],dict["type"]  = timestamp,event["x"],event["y"],event["polarity"],string
            list.append(dict)
            dict = {}

ft.writeJson(list,"dvSaveTotEvents",cls=ft.NumpyEncoder)
